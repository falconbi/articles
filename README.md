...
- **[Self-referencing COA to tiered reporting dimensions](https://falconbi.github.io/articles/Hierarchy%20Patterns/coa-to-reporting-tiers.html)** — full implementation pattern for converting a flat, self-referencing chart of accounts into standalone reporting-level dimensions: frame model, leaf detection, composite mapping, and a validation net — for tools without native ragged consolidation.
...
- **[Pigment and TM1: matching the engine to the work](https://falconbi.github.io/articles/pigment-vs-tm1-plain.html)** — a plain-language comparison of the two calculation engines. How each scales, how each handles calculations that depend on their own result, and an interactive cost-of-change matrix covering hierarchy reshaping, logic change, scale, sharing, and circular calculations.
